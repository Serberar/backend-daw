...
module.exports = async function register({ firstName, lastName, dni, phone, address, email, password }) {
  if (await userRepo.findByEmail(email)) throw err(`El usuario '${email}' ya existe`, 409);

  const passwordHash = await bcrypt.hash(password, 12);
  const user = User.create({ firstName, lastName, dni, phone, address, email, passwordHash });
